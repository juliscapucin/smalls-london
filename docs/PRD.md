...
- Radix UI (dependencies.@base-ui/react or @base-ui-components/react)
...
```tsx
focus-visible:outline-none focus-visible:ring-2 ring-focus-ring ring-offset-2
```
...
- If a component exists in the Radix UI library, use it
...
1. Use components from `/src/components/ui`
2. Use token-based values only for typography and colors
3. Use Tailwind tokens for spacing
...
This project uses a shadcn-style architecture built on top of Radix UI components and `class-variance-authority` (CVA) where appropriate.
...
- use Radix UI primitives where relevant
...
- use existing `/src/components/ui` components first
...
- match the implementation pattern already established in `/src/components/ui`
